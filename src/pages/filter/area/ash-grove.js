...
const FilterAshGrovePage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Ash Grove area Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Ash Grove"{" "}
...
export const pageQuery = graphql`
  query FilterAshGroveQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "ash grove" } } }
    ) {
      edges {
        node {
          id
...
export default FilterAshGrovePage
