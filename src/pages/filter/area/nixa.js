...
const FilterNixaPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Nixa area Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Nixa"{" "}
...
export const pageQuery = graphql`
  query FilterNixaQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "nixa" } } }
    ) {
      edges {
        node {
          id
