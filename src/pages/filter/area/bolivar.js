...
const FilterBolivarPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Bolivar area Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Bolivar"{" "}
...
export const pageQuery = graphql`
  query FilterBolivarQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "bolivar" } } }
    ) {
      edges {
        node {
          id
...
export default FilterBolivarPage
