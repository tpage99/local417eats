...
const FilterMarshfieldPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Marshfield area Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Marshfield"{" "}
...
export const pageQuery = graphql`
  query FilterMarshfieldQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "marshfield" } } }
    ) {
      edges {
        node {
          id
...
export default FilterMarshfieldPage
