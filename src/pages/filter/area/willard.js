...
const FilterWillardPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Willard area Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Willard"{" "}
...
export const pageQuery = graphql`
  query FilterWillardQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "willard" } } }
    ) {
      edges {
        node {
          id
...
export default FilterWillardPage
