...
const FilterOzarkPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Ozark area Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Ozark"{" "}
...
export const pageQuery = graphql`
  query FilterOzarkQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "ozark" } } }
    ) {
      edges {
        node {
          id
...
export default FilterOzarkPage
