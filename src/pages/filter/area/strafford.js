...
const FilterStraffordPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Strafford area Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Strafford"{" "}
...
export const pageQuery = graphql`
  query FilterStraffordQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "strafford" } } }
    ) {
      edges {
        node {
          id
...
export default FilterStraffordPage
