...
const FilterTakeoutPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by takeout ordering options" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "takeout" ordering options{" "}
...
export const pageQuery = graphql`
  query FilterTakeoutQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { orderops: { eq: "takeout" } } }
    ) {
      edges {
        node {
          id
...
export default FilterTakeoutPage
