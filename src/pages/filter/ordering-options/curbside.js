...
const FilterCurbsidePage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by curbside ordering options" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "curbside" ordering options{" "}
...
export const pageQuery = graphql`
  query FilterCurbsideQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { orderops: { eq: "curbside" } } }
    ) {
      edges {
        node {
          id
...
export default FilterCurbsidePage
