...
const FilterOrderOnlinePage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by order online ordering options" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "order online" ordering options{" "}
...
export const pageQuery = graphql`
  query FilterOrderOnlineQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { orderops: { eq: "order online" } } }
    ) {
      edges {
        node {
          id
...
export default FilterOrderOnlinePage
