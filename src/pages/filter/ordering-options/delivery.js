...
const FilterDeliveryPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by delivery orderin options" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "delivery" ordering options{" "}
...
export const pageQuery = graphql`
  query FilterDeliveryQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { orderops: { eq: "delivery" } } }
    ) {
      edges {
        node {
          id
...
export default FilterDeliveryPage
