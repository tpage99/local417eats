...
const FilterDineInPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by dine-in ordering options" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "dine-in" ordering options{" "}
...
export const pageQuery = graphql`
  query FilterDineInQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { orderops: { eq: "dine-in" } } }
    ) {
      edges {
        node {
          id
...
export default FilterDineInPage
