...
const FilterCoffeePage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Coffee Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Coffee"{" "}
...
export const pageQuery = graphql`
  query FilterCoffeeQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "coffee" } } }
    ) {
      edges {
        node {
          id
...
export default FilterCoffeePage
