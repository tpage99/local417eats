...
const FilterPizzaPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Pizza Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Pizza"{" "}
...
export const pageQuery = graphql`
  query FilterPizzaQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "pizza" } } }
    ) {
      edges {
        node {
          id
...
export default FilterPizzaPage
