...
const FilterDonutsPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Donuts Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Donuts"{" "}
...
export const pageQuery = graphql`
  query FilterDonutsQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "donuts" } } }
    ) {
      edges {
        node {
          id
...
export default FilterDonutsPage
