...
const FilterCChickenPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Cashew Chicken Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Cashew Chicken"{" "}
...
export const pageQuery = graphql`
  query FilterCChickenQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "cashew chicken" } } }
    ) {
      edges {
        node {
          id
...
export default FilterCChickenPage
