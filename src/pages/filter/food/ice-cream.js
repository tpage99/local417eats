...
const FilterIceCreamPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Ice Cream Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Ice Cream"{" "}
...
export const pageQuery = graphql`
  query FilterIceCreamQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "ice cream" } } }
    ) {
      edges {
        node {
          id
...
export default FilterIceCreamPage
