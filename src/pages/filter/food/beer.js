...
const FilterBeerPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Beer Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Beer"{" "}
...
export const pageQuery = graphql`
  query FilterBeerQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "beer" } } }
    ) {
      edges {
        node {
          id
