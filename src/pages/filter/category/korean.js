...
const FilterKoreanPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Korean Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Korean"{" "}
...
export const pageQuery = graphql`
  query FilterKoreanQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "korean" } } }
    ) {
      edges {
        node {
          id
...
export default FilterKoreanPage
