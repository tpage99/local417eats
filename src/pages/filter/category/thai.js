...
const FilterThaiPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Thai Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Thai"{" "}
...
export const pageQuery = graphql`
  query FilterThaiQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "thai" } } }
    ) {
      edges {
        node {
          id
