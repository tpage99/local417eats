...
const FilterBBQPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by BBQ Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "BBQ"{" "}
...
export const pageQuery = graphql`
  query FilterBBQQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "bbq" } } }
    ) {
      edges {
        node {
          id
