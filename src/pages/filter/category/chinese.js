...
const FilterChinesePage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Chinese Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Chinese"{" "}
...
export const pageQuery = graphql`
  query FilterChineseQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "chinese" } } }
    ) {
      edges {
        node {
          id
...
export default FilterChinesePage
