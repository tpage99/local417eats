...
const FilterSandwichShopPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Sandwich Shop Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Sandwich Shop"{" "}
...
export const pageQuery = graphql`
  query FilterSandwichShopQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "sandwiches" } } }
    ) {
      edges {
        node {
          id
...
export default FilterSandwichShopPage
