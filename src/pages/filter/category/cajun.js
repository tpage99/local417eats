...
const FilterCajunPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Cajun Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Cajun"{" "}
...
export const pageQuery = graphql`
  query FilterCajunQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "cajun" } } }
    ) {
      edges {
        node {
          id
...
export default FilterCajunPage
