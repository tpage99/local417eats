...
const FilterFoodTruckPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Food Truck Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Food Truck"{" "}
...
export const pageQuery = graphql`
  query FilterFoodTruckQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "food truck" } } }
    ) {
      edges {
        node {
          id
...
export default FilterFoodTruckPage
