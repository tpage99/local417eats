...
const FilterBreakfastPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Breakfast Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Breakfast"{" "}
...
export const pageQuery = graphql`
  query FilterBreakfastQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "breakfast" } } }
    ) {
      edges {
        node {
          id
...
export default FilterBreakfastPage
