...
const FilterAsianFusionPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Asian Fusion Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Asian Fusion"{" "}
...
export const pageQuery = graphql`
  query FilterAsianFusionQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "asian fusion" } } }
    ) {
      edges {
        node {
          id
...
export default FilterAsianFusionPage
