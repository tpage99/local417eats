...
const FilterBakeryPage = ({ data }) => (
  <FoodLayout>
    <SEO title="Filter by Bakery Eats" />
    <h1 className="mb-0 py-2 text-4xl font-bold text-center leading-tight">
      Filtered results for "Bakery"{" "}
...
export const pageQuery = graphql`
  query FilterBakeryQuery {
    allMarkdownRemark(
      sort: { fields: [frontmatter___title], order: ASC }
      filter: { frontmatter: { tags: { eq: "bakery" } } }
    ) {
      edges {
        node {
          id
...
export default FilterBakeryPage
